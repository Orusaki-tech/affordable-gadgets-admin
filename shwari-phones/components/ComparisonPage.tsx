...
import { PublicProduct, PublicInventoryUnitPublic } from '@/lib/api/generated';
...
function PriceCell({ product, allProducts }: { product: PublicProduct; allProducts: PublicProduct[] }) {
  ...
  const allPrices = allProducts.map((p) => {
    ...
    const max = p.max_price;
    const hasRange = typeof min === 'number' && typeof max === 'number';
    return hasRange ? (min === max ? min : (min + max) / 2) : null;
  });
...
function UnitAttributeCell({ 
  ...
  if (attributeType === 'condition') {
    const conditions = Array.from(new Set(unitList.map(u => u.condition).filter(Boolean)));
  ...
  if (attributeType === 'grade') {
    const grades = Array.from(new Set(unitList.map(u => u.grade).filter(Boolean)));
    return (
      <td className="border-b border-gray-100 p-4">
        <div className="flex flex-wrap gap-1">
  ...
  if (attributeType === 'colors') {
    const colors = Array.from(new Set(unitList.map(u => u.color_name).filter(Boolean)));
    return (
      <td className="border-b border-gray-100 p-4">
        <div className="flex flex-wrap gap-1">
          {colors.length > 0 ? (
...
export function ComparisonPage() {
  ...
  const selectedProductsData = useMemo(() => {
    ...
    return results.filter((p) => typeof p.id === 'number' && compareList.includes(p.id));
  ...
  const filteredProducts = useMemo(() => {
    const results = productsData?.results ?? [];
    return results
      .filter((p) => typeof p.id === 'number' && !compareList.includes(p.id))
      .filter((p) => {
